...
const FormPengajuanUJK = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setAlert({
      ...
      text: 'Pastikan seluruh dokumen dan data peserta sudah benar.',
      onConfirm: () => {
        const newUsulanList = skemaUsulan.map((s, index) => {
          ...
          return {
            id: Date.now() + index, nomorSurat: formData.nomorSurat, pendanaan: formData.pendanaan, tuk: formData.tuk,
        ...
        setUsulan([...usulan, ...newUsulanList]);
        setShowForm(false);
      ...
      {selectedPeserta ? (
        <div className="fade-in-content">
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '20px', flexWrap: 'wrap' }}>
            ...
            <div>
              <h2 style={{ margin: 0, fontSize: '1.5rem', color: '#0f172a' }}>Data Peserta Ujian</h2>
        ...
        <div className="fade-in-content">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
            <div>
              <h2 style={{ margin: '0 0 5px 0', color: '#0f172a' }}>Pengajuan UJK Baru</h2>
              <p className="text-muted" style={{ margin: 0 }}>Isi formulir bertahap di bawah ini untuk mengajukan jadwal ujian kompetensi baru.</p>
            </div>
            <div className="page-header-actions">
              <Button variant={showForm ? 'danger' : 'primary'} icon={showForm ? 'times' : 'plus'} size="lg" onClick={() => setShowForm(!showForm)}>
                {showForm ? 'Batal Mengajukan' : 'Buat Pengajuan Baru'}
              </Button>
            </div>
          </div>
              ...
              <form className="admin-form" onSubmit={handleSubmit}>
                ...
                <div className="form-section">
                  ...
                  <div className="form-group">
                    ...
                    <small className="text-muted"><i className="fas fa-info-circle"></i> Wajib menggunakan format .pdf (Maks. 5MB)</small>
                  </div>
                </div>
                ...
                <div className="form-section">
                  ...
                  <div className="mb-30 mt-20">
                    <Button variant="dashed" size="lg" isFullWidth icon="plus-circle" onClick={handleAddSkema}>Tambah Skema Ujian Lainnya</Button>
                  </div>
                </div>
                ...
                <tbody>
                  {usulan.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        {item.status === 'Pending' && (
                           <Button variant="danger" size="sm" onClick={() => handleDelete(item.id)}>Batalkan</Button>
                        )}
                      </td>
                    </tr>
                  ))}
