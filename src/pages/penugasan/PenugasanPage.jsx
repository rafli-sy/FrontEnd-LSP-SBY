...
const PenugasanPage = () => {
  ...
  const handleDocClick = (jenisSurat, suratItem, skemaItem, docKey) => {
    ...
    if (jenisSurat === 'Administrasi Pleno' || docKey === 'administrasiPleno') {
      ...
      if (!isComplete) {
        showAlert('warning', 'Data Belum Lengkap', 'Silakan tentukan Asesor Penguji dan Keputusan Uji untuk semua asesi terlebih dahulu di menu "Pembagian Asesi".');
        return;
      }
    }
  ...
  const handleSubMenuClick = (doc) => {
    if (doc.code === 'DOC.01') {
      ...
      if (!isAsesorComplete) {
        showAlert('warning', 'Data Belum Lengkap', 'Silakan tentukan Asesor Penguji untuk semua asesi terlebih dahulu di menu "Pembagian Asesi".');
        return;
      }
    }
  ...
  const mapPdfEndpoint = (docKey, skemaId, pengajuanId, isTtd = false) => {
    ...
    const map = {
      ...
      'SPT.02': `/${rolePath}/cetak-surat-spt-asesor${ttdSuffix}/${skemaId}`,
