...
const PenugasanPage = () => {
  ...
  const mapPdfEndpoint = (docKey, skemaId, pengajuanId, isTtd = false) => {
    const map = {
      'balasan': `/${rolePath}/cetak-surat-balasan/${pengajuanId}`,
      'SPT.01': `/${rolePath}/cetak-surat-spt-asesor/${skemaId}`,
      'SPT.02': `/${rolePath}/cetak-surat-spt-asesor/${skemaId}`,
      'SPT.03': `/${rolePath}/cetak-surat-spt-penyilia/${skemaId}`,
      'SPM.01': `/${rolePath}/cetak-surat-permohonan-asesor1/${skemaId}`,
      'SPM.02': `/${rolePath}/cetak-surat-permohonan-asesor2/${skemaId}`,
      ...
      {isLoading ? (
          ...
          return (
            <div className="fade-in-content">
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '25px', paddingBottom: '15px', borderBottom: '1px solid #e2e8f0' }}>
                ...
                <div><h2 style={{ margin: 0, fontSize: '1.5rem', color: '#0f172a' }}>{isStaffView ? "Manajemen Dokumen & Administrasi UJK" : "Manajemen Penugasan UJK"}</h2><p className="text-muted" style={{ margin: 0 }}>{activeSurat.idUjk} - {activeSurat.pengusul}</p></div>
              </div>
              ...
              <div className="dashboard-card" style={{ backgroundColor: '#fff', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '20px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
                ...
                {!isSemuaDiplot ? (
                  <div style={{ background: '#fff7ed', color: '#ea580c', padding: '8px 14px', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid #fed7aa' }}>
                    <i className="fas fa-lock"></i> {isStaffView ? "Menunggu Admin LSP menyelesaikan Plotting!" : "Selesaikan semua plotting skema terlebih dahulu!"}
                  </div>
                ) : (
                      ...
                      <div style={{ textAlign: 'right' }}>
                        {!isFullyPlotted ?
                          <span style={{ backgroundColor: '#fff7ed', color: '#ea580c', padding: '6px 12px', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 'bold', border: '1px solid #fed7aa' }}>
                            <i className="fas fa-clock"></i> {isStaffView ? "Menunggu Plotting Admin" : "Menunggu Plotting"}
                          </span>
                          :
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', backgroundColor: '#f8fafc', padding: '16px', borderRadius: '10px', border: '1px solid #e2e8f0', marginBottom: '25px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px', background: '#ffffff', border: needsAsesiAction ? '2px dashed #f59e0b' : '1px solid #cbd5e1', borderRadius: '10px', marginTop: '5px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                          ...
                          >
                            {isStaffView ? 'Lihat Asesi' : (needsAsesiAction ? 'Plotting Asesi ->' : 'Kelola')}
                          </button>
                        </div>
                      </div>
                    </div>
                      ...
                      <div>
                        {isEditing ? (
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            ...
                            {!isEditing && !skema.isPlotted && !isStaffView && (
                        ...
                        {!isEditing && (
                          <>
                            {[
                              ...
                              { id: 'administrasi', title: 'Administrasi', status: isAdministrasiDone, key: 'administrasi', type: 'Administrasi' },
        ...
        <div className="fade-in-content">
          ...
          <div className="dashboard-header" style={{ marginBottom: '25px' }}>
            <h2 style={{ fontSize: '1.8rem', color: '#1e293b', marginBottom: '8px' }}>{isStaffView ? "Manajemen Dokumen & Surat Menyurat" : "Manajemen Penugasan (Plotting)"}</h2>
            <p className="text-muted">{isStaffView ? "Halaman khusus Staf LSP untuk mengelola administrasi, mengunduh PDF berkas uji, dan meneruskan surat balasan." : "Atur jadwal pelaksanaan, distribusikan Asesor, dan terbitkan dokumen surat persetujuan LSP."}</p>
          </div>
                      ...
                      <tr key={item.idUjk} style={{ backgroundColor: isHighlighted ? '#fffbeb' : 'inherit', borderLeft: isHighlighted ? '4px solid #f59e0b' : 'none', transition: 'all 0.3s ease' }}>
                        ...
                        <td style={{ textAlign: 'center', verticalAlign: 'top', paddingTop: '15px' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center' }}>
                            ...
                            >
                              {isSemuaDiplot ? 'Kelola Dokumen' : (isStaffView ? 'Lihat Detail' : 'Mulai Plotting')}
                            </button>
                          </div>
                        </td>
