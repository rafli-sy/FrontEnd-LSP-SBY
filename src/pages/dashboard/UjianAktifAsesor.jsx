...
const UjianAktifAsesor = () => {
  // Mock data detail ujian (Berdasarkan aturan Single Task)
  const detailUjian = {
  ...
  const [peserta, setPeserta] = useState([
    { id: 1, nik: '3578012345670001', nama: 'Giyu Tomioka', nilaiPraktik: '', rekomendasi: '' },
  ...
  // Handler untuk mengubah input nilai
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Di sini nantinya logika untuk mengirim data (POST/PUT) ke backend (PostgreSQL)
                  ...
                  <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '12px' }}>
                      <select
                        ...
                        value={p.rekomendasi}
                        onChange={(e) => handleInputChange(p.id, 'rekomendasi', e.target.value)}
                        required
                      >
