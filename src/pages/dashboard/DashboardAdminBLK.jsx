...
const DashboardAdminBLK = () => {
  ...
  const itemsPerPage = 6; // Maksimal 6 data per halaman

  // --- DATA SIMULASI (Sengaja dibuat 8 data agar pagination terlihat) ---
  const riwayatPengajuan = [
    ...
    { 
      ...
      peserta: [
        ...
        { id: 2, nama: 'Budi Santoso', nik: '3578902006900002', jk: 'L', tempatLahir: 'Malang', tanggalLahir: '15 Agustus 1995', alamat: 'Jl. Raya Mondoroko No 1', rt: '2', rw: '3', kelurahan: 'Singosari', kecamatan: 'Singosari', hp: '081234567890', email: 'budi@gmail.com', pendidikan: 'SMA' }
      ]
    },
  ...
  // 1. Hitung urutan awal dan akhir data untuk halaman saat ini
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  
  // 2. Ambil data spesifik untuk dihalaman tersebut
  const currentItems = riwayatPengajuan.slice(indexOfFirstItem, indexOfLastItem);
  
  // 3. Hitung total halaman yang dibutuhkan
  const totalPages = Math.ceil(riwayatPengajuan.length / itemsPerPage);
  ...
  return (
    <div className="dashboard-content fade-in-content">
      ...
      {/* === LAYAR 1: PENINJAU PDF SURAT BALASAN === */}
      {previewBalasan ? (
        <div className="fade-in-content">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                <Button variant="outline" icon="arrow-left" onClick={() => setPreviewBalasan(null)}>
                  Kembali
                </Button>
                <div>
            ...
            <Button variant="primary" icon="download" onClick={() => alert('Mengunduh PDF Surat Balasan...')}>
              Unduh PDF
            </Button>
          </div>
          ...
          <div className="dashboard-card" style={{ minHeight: '60vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f8fafc', border: '2px dashed #cbd5e1' }}>
             <div style={{ textAlign: 'center', color: '#94a3b8' }}>
               ...
               <p>Area ini akan menampilkan file surat balasan resmi dari LSP.</p>
             </div>
          </div>
        </div>
        ...
        /* === LAYAR 2: DETAIL PESERTA (TABLE PESERTA) === */
        <div className="fade-in-content">
        ...
        /* === LAYAR 3: DASHBOARD UTAMA BLK (DAFTAR PENGAJUAN) === */
        <div className="fade-in-content">
          
          <div className="stats-grid">
            <div className="stat-card">
              <div className="stat-icon" style={{ background: '#eff6ff', color: '#1d4ed8' }}><i className="fas fa-paper-plane"></i></div>
              <div className="stat-info">
                <h3>{totalUsulan}</h3>
                <p>Total Usulan UJK</p>
              </div>
            </div>
            <div className="stat-card">
          ...
          <div className="dashboard-card" style={{ marginTop: '25px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
              <div>
                <h3 style={{ margin: '0 0 5px 0', fontSize: '1.3rem', color: '#0f172a' }}>Riwayat Pengajuan Ujian</h3>
                <p className="text-muted" style={{ margin: 0 }}>Daftar status permohonan dan dokumen balasan dari LSP (Terbaru di atas).</p>
              </div>
            ...
            <div className="table-responsive">
              <table className="admin-table">
                ...
                <tbody>
                  {/* MEMAPPING DATA currentItems, BUKAN SEMUA DATA */}
                  {currentItems.map((item, index) => (
                    <tr key={item.id}>
                      {/* Penomoran dinamis berdasarkan halaman */}
                      <td style={{ textAlign: 'center', color: '#64748b' }}>{indexOfFirstItem + index + 1}</td>
                      <td>
                        <strong style={{ color: '#0f172a' }}>{item.skema}</strong><br/>
                        <span className="text-muted" style={{ fontSize: '0.85rem' }}>{item.kejuruan}</span>
                      </td>
                      <td>
                        <span style={{ fontWeight: '600', color: '#334155' }}>{item.tglMulai}</span> <br/>
                        <span className="text-muted" style={{ fontSize: '0.85rem' }}>s/d {item.tglSelesai}</span>
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <Button variant="outline" size="sm" onClick={() => setSelectedPeserta(item)}>
                          <strong>{item.peserta?.length || 0}</strong> Orang <i className="fas fa-users" style={{ marginLeft: '5px' }}></i>
                        </Button>
                      </td>
                      <td>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '8px' }}>
                          ...
                          {item.status === 'Ditolak oleh LSP' && (
                            <span className="text-muted" style={{ fontSize: '0.8rem' }}>
                              <i className="fas fa-info-circle"></i> Silakan ajukan ulang
                            </span>
                          )}
                        </div>
                      </td>
            ...
            {/* === KONTROL PAGINATION DI BAWAH KIRI TABEL === */}
            {totalPages > 1 && (
              <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', marginTop: '20px', gap: '8px' }}>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                  <Button
                    ...
                    style={{ minWidth: '35px', padding: '6px 10px', fontSize: '0.9rem' }}
                  >
                    {page}
                  </Button>
                ))}
